...
void serialBoruvkaMST(struct Graph* graph)
{
	...
	while (numTrees > 1)
	{
		...
		#pragma omp parallel for default(none) shared(cheapest)
		for (int v = 0; v < V; v++)
		{
			cheapest[v] = -1;
		}
...
//		#pragma omp parallel for// default(none) shared(V, numTrees, MSTweight, cheapest)
		for (int v = 0; v < V; v++)
		{
...
int main()
{
	...
	//std::cin >> V >> E;
	printf("OpenMP Minimum Spanning Tree Program\n\n");
